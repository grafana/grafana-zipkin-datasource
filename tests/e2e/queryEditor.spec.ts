...
import { DS_NAME, isCloudRun, resolveDataSourceUid } from './env';
...
function exploreUrl(traceID: string, dsUid: string): string {
      ...
      queries: [
        {
          ...
          query: traceID,
          queryType: 'traceID',
          datasource: { type: 'zipkin', uid: dsUid },
        },
      ],
...
test.describe('Query editor', () => {
  ...
  test.describe('rendering', () => {
    test('smoke: renders query type controls', { tag: '@plugins' }, async ({ explorePage, page }) => {
  ...
  test.describe('query execution', () => {
    test('executes a trace id query and receives OK response', async ({ explorePage, page }) => {
      ...
      await page.goto(exploreUrl(FRONTEND_TRACE_ID, dsUid));
...
test.describe('Query editor with fixture data', () => {
  ...
  test.beforeEach(() => {
    test.skip(
      isCloudRun,
      'Asserts on trace ids generated by the zipkin-loader service in docker-compose.yaml, which do not exist in the Cloud backend.'
    );
  });
  ...
  test.describe('backend trace', () => {
    test(`returns 1 span for trace ${BACKEND_TRACE_ID}`, async ({ page }) => {
      const dsUid = await resolveDataSourceUid(page);
      const responsePromise = waitForMainQueryResponse(page);
      await page.goto(exploreUrl(BACKEND_TRACE_ID, dsUid));
      const { response, body } = await responsePromise;
      expect(response.ok()).toBe(true);
      expect(body.results?.A?.error).toBeUndefined();
      expect(body.results?.A?.frames?.length).toBeGreaterThan(0);
